...
defmodule NaiveDateTime.Range do
  @moduledoc """
  ...
  @doc ~S"""
  Create a new NaiveDateTime Range

  ## Examples

      iex> NaiveDateTime.Range.new(DateTime.from_unix!(0), DateTime.from_unix!(1))
      %NaiveDateTime.Range{
  ...
  @impl Ecto.Type
  def cast(term)
  def cast(%NaiveDateTime.Range{} = range), do: {:ok, range}
  def cast(_), do: :error
  ...
  @impl Ecto.Type
  def dump(%NaiveDateTime.Range{} = range) do
...
defimpl Inspect, for: NaiveDateTime.Range do
  # TODO: Replicate the representation from Postgres ? (with `[]` and `()`).
  def inspect(%NaiveDateTime.Range{first: first, last: last}, _) do
    "#NaiveDateTime.Range<" <> inspect(first) <> ", " <> inspect(last) <> ">"
  end
end
